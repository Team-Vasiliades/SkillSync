...
# View function for resume suggestions
    ...
    if request.method == 'POST' and request.FILES.get('resume'):
        ...
        try:
            ...
            response = model.generate_content([
                "Suggest improvements for this resume in a structured format using Markdown:",
                resume_content
            ])
            
            raw_suggestions = response.text  # AI response

            # Convert Markdown to HTML
            formatted_suggestions = markdown.markdown(raw_suggestions)

        except Exception as e:
            return render(request, 'suggest.html', {"error": f"Error processing file: {str(e)}"})

        return render(request, 'suggest.html', {"suggestions": formatted_suggestions})

    return render(request, 'suggest.html', {"error": "Please upload a valid file."})


def clean_text(text):
    text = re.sub(r'\*\*|\*|_', '', text)  # Remove **, *, and _ (Markdown formatting)
    text = re.sub(r'[\u2022\u25CF]', '-', text)  # Replace bullet points with "-"
    return text.strip()
...
def convert_to_cv(request):
    if request.method == 'POST' and request.FILES.get('resume'):
        ...
        try:
            ...
            # Generate a structured CV using Gemini AI
            response = model.generate_content([
                "Generate a professional CV from this resume content, with properly formatted sections like Name, Contact, Summary, Work Experience, Education, Skills, and Projects (Do NOT use special characters like ** or * in formatting) and expand every data of resume in wording so that it look like cv:",
                resume_content
            ])
            structured_cv = clean_text(response.text)

            # Create a CV document using python-docx
            doc = Document()
            doc.add_heading('Curriculum Vitae', level=1)

            # Format AI-generated text into the document
            for section in structured_cv.split("\n\n"):
                if ":" in section:
...
from django.shortcuts import render
from asgiref.sync import sync_to_async
from .utils import Scrap_Internshala, scrap_jobs  # Import utility functions

# API Keys
SERPAPI_KEY = "b1b74ec92d0858703b2a9bb9b5e0ac21746fd3bae2b9359dc6cedd581008242e"
DEEPSEEK_API_KEY = "sk-84066dd6566b41c8b6961bbcc234d535"

# -------- USA Jobs API Fetching --------
class USAJobsView:
    ...
    async def get_jobs_data(self):
        ...
        api_key_1 = '3WGgHaa+SeqrF14ap76WXFHc1znhD8stg7bEJhkC6Y8='
        api_key_2 = '2af21edf-fc6b-4d13-9cd8-17bba4a7ce1d'
...
async def merged_view(request):
    ...
    if request.method == "POST":
        ...
        if skill:
            usa_jobs_view = USAJobsView()
            usa_jobs = await usa_jobs_view.get_usa_jobs()
            internships = fetch_internships(skill)
            jobs = fetch_jobs(skill)
            google_jobs = fetch_jobs_from_google(skill)
